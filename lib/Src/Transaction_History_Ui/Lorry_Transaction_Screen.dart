...
class Lorry_Transaction_Screen extends ConsumerStatefulWidget {
  List<ListData> sitenameData = [];

  Lorry_Transaction_Screen(this.sitenameData, {super.key});

  @override
  ConsumerState<Lorry_Transaction_Screen> createState() =>
      _Lorry_Transaction_ScreenState();
}

class _Lorry_Transaction_ScreenState
    extends ConsumerState<Lorry_Transaction_Screen> {
  ...
  getTransactionList(String site_id) async {
    ...
    final postResponse = await apiService.post<CommonListModel>(
        ConstantApi.lorry_transaction_list, formData);
    if (postResponse.success == true) {
      setState(() {
